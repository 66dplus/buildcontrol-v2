...
3. Закупки — строго различай:
   ...
   Перерасход в закупках — запускай ВСЕ ТРИ проверки (см. примеры запросов в схеме):
     а) json_each(items_json) WHERE price > price_plan     ← перерасход цены в заявках
     б) materials WHERE price_actual > price_plan           ← перерасход средней цены
     в) consumed_pct >> task_done_pct + 15%                 ← мягкий сигнал темпа расхода
   НЕ используй status = 'rejected' как признак перерасхода — это разные вещи.
   При перерасходе цены:
   — Любое превышение price > price_plan = перерасход, даже 0.1%. Не округляй до нуля.
   — Указывай ИТОГОВЫЕ суммы (qty × price), а не цены за единицу.
   — Формат: «По цене есть перерасход: {{overspend}} ₽. По плану {{plan_total}} ₽, закупка на {{actual_total}} ₽ (+{{dev_pct}}%).»
